# 🚀 LeetCode Challenges - My Coding Journey

This repository is dedicated to tracking my progress as I solve algorithm and data structure problems on LeetCode.

## 📌 Challenges Solved
Below is a list of problems I’ve completed, with links to the solutions and their respective LeetCode problem pages:

| #  | Problem Name          | Solution | LeetCode Link |
|----|----------------------|----------|--------------|
| 1  | Two Sum             | [View](https://github.com/danielnserpa/leetcode-exercises/blob/main/TwoSum.java) | [LeetCode](https://leetcode.com/problems/two-sum/) |
| 2  | Roman to Integer    | [View](https://github.com/danielnserpa/leetcode-exercises/blob/main/RomanToInt.java) | [LeetCode](https://leetcode.com/problems/roman-to-integer/) |
| 3  | Palindrome Number   | [View](https://github.com/danielnserpa/leetcode-exercises/blob/main/Palyndrome.java) | [LeetCode](https://leetcode.com/problems/palindrome-number/) |
| 4  | Single Number       | [View](https://github.com/danielnserpa/leetcode-exercises/blob/main/SingleNumber.java) | [LeetCode](https://leetcode.com/problems/single-number/) |
| 5  | Missing Number      | [View](https://github.com/danielnserpa/leetcode-exercises/blob/main/MissingNumber.java) | [LeetCode](https://leetcode.com/problems/missing-number/) |
| 6  | Search Insert Position | [View](https://github.com/danielnserpa/leetcode-exercises/blob/main/InsertPosition.java) | [LeetCode](https://leetcode.com/problems/search-insert-position/) |
| 7  | Search String in a String | [View](https://github.com/danielnserpa/leetcode-exercises/blob/main/FirstOccurrenceInAString.java) | [LeetCode](https://leetcode.com/problems/find-the-index-of-the-first-occurrence-in-a-string/) |
